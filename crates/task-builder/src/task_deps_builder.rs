...
pub trait TasksQuerent {
    /// The container type returned by methods that return collections of project IDs.
    /// Must be iterable over `&Id` references.
    type IdsCollection<'a>: IntoIterator<Item = &'a Id>
    where
        Self: 'a;
    ...
    /// Returns the precomputed transitive dependencies for a project.
    /// Errors if transitive dependencies were not precomputed.
    fn query_transitive_deps(&self, project_id: &Id) -> miette::Result<Self::IdsCollection<'_>>;
}

pub struct TaskDepsBuilder<'proj, Q: TasksQuerent> {
    pub querent: Q,
...
impl<Q: TasksQuerent> TaskDepsBuilder<'_, Q> {
            ...
            let (project_ids, skip_if_missing, link_implicit_project_deps) =
                match &dep_config.target.scope {
                    ...
                    TargetScope::TransitiveDeps => {
                        let transitive_dep_ids: Vec<&Id> = self
                            .querent
                            .query_transitive_deps(&project.id)?
                            .into_iter()
                            .collect();
